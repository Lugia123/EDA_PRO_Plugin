...
## §4.10 电源地符号的扇出：三次尝试都失败了，记下来

芯片同一侧的引脚间距只有 10（AMS1117 左侧 ADJ/VOUT/VIN 是 y=210/200/190），
而电源地符号连文字有几十宽。三个符号直接挂上去必然叠成一团 —— 需要像
PCB 的 fan-out 那样，从密集引脚按次序逐级张开再接符号。

目前**没有做到**。当前实现是「符号直接落在引脚上」，视觉上会重叠，但电气
是干净的（体检全过）。三次尝试的失败方式各不相同，都值得记住：

| 做法 | 结果 |
|---|---|
| 沿引脚方向直着拉，长度按序递增 | 一排水平引脚的 stub 全落在同一条 y 线上首尾相接，几条网络被并成一个 —— **真短路**，DRC 不报 |
| 改 L 形转向 | 转折点 `mx` 固定，垂直段全在同一条 x 上，照样接成一条，反而并了五条网络 |
| 对称扇形（水平段与垂直段都按次序张开）＋落点占位＋路径占位 | 符号确实分开了，但仍有 2 处短路 |

第三次的具体障碍已经查到：**有扇出线朝器件内部走**。C2 竖放时两个引脚在
y=210 和 y=170，中间出现一条 `[1140,210 - 1140,170]` 的线，直接把电容两极
短接。方向来自 `dirVec(pinWorld().dir)`，说明**引脚的 dir 在某些旋转下给出
的是朝内的方向** —— 这和 pinWorld 的位置计算是两回事，位置已经修对了
（§0.铁律，先旋转后镜像），dir 还没有对应的实测验证。

顺带确认的两件事，避免重复排查：

- `sch_PrimitiveWire.create` **不吸附坐标**（传 2570 读回 2570），
  但 `createNetFlag` **会吸附到 5 的倍数**。扇出算出 917.5 而符号被吸到
  920，差 5 刚好连不上。所以扇出坐标必须自己先对齐到 5 的倍数。
- 电源地符号和跨区端口**必须一起归组扇出**。它们挂在同一排引脚上，
  各扇各的必然撞。

### 正解的方向

贪心避让（撞了就往外让）治不了根，因为扇出线不知道 A\* 画的信号线在哪、
也不知道别的器件的引脚在哪。真正干净的做法是**把符号和端口当作虚拟器件，
它们的引出线交给 route() 和信号线一起布**，共用同一套 blocked / ownPins /
occupied。route.ts 已经有完整的 A\*，缺的是把符号纳入布线模型。

在那之前，宁可让符号叠在一起 —— 可读性差是看得见的，短路是看不见的。
