...
所以**不能拿朝向去躲邻居**。但朝向也不是一个常数 —— 它由所处场景决定，
分两种，判据是**簇里有几个引脚**（簇 = 同一器件、同一引出方向的一组引脚）：

**独苗（簇内 1 个）→ 语义朝向。** 两脚阻容的电源地脚就是这种。前方没有
邻居的引出线，不存在撞不撞的问题，画成语义正确的姿势：Power 本体朝上、
Ground 本体朝下。

**密集扇出（簇内 ≥2 个）→ 本体一律伸向「阶梯变短」的那一侧。** 水平引出
是朝下，垂直引出是朝左。这条不看周围有没有空间，由阶梯方向唯一确定。

这两条不能混用。用阶梯规则去套独苗，电容竖放时 +3V3 符号落在 pin1 上方
40 处、本体再往下伸 45，正好压回电容身上。

#### 为什么密集扇出要全部朝同一侧

符号本体约 45 长，而同侧相邻引脚只差 10 —— 本体一定会跨过邻居那根引出线
所在的高度。撞不撞，只取决于邻居的线**够不够长、能不能延伸到本体所在的 x**：

| 本体伸向 | 那侧的线 | 结果 |
|---|---|---|
| 阶梯变短的一侧 | 更短，左端还没够到本体 | 安全 |
| 阶梯变长的一侧 | 更长，必然横跨本体 | 必撞 |

阶梯是「从上到下由长到短」，所以水平引出时**全部朝下**就无冲突。这正是
那条长度规则存在的理由，两者是配套的。

代价是电源符号会倒过来画（单横杠 `⊥`）。它和地符号（三条递减横杠）形状
不同，旁边还有网络名，图上不会误读。

曾经在这里绕了很久：先按「器件总引脚数」分流，再按「同侧引脚数」分流，
让多引脚那侧的符号「背离引脚簇」，转出来一堆倒着的地符号；后来改成按语义
固定 Power 朝上、Ground 朝下，同一侧就变成一上一下 —— 朝下的那些安全，
朝上的那些（U1 的 VDDA、VDD_1）本体必然穿过上方那根更长的线。当时误判成
「一上一下必有一个撞」的死结，还打算改成引出后拐弯。其实两侧并不对称：
**全部朝下就同时避开了**，纯直线足够，不需要拐弯。

教训是：判据一直在往「更细的条件分支」上走，而正解是让朝向由已有的阶梯
规则推出来 —— 规则之间该有推导关系，不该各定各的然后打架。

#### 每种符号的原生朝向不同，没有通用的角度映射

这里栽过一次，教训值得单列。曾经拿地符号量出「0=朝下、90=朝左、180=朝上、
270=朝右」，当成 `createNetFlag` 的**通用**映射，于是给 Power 写 180 去求
「朝上」。结果电源符号被翻了个个儿，图上电源和地全都朝下 —— 地是对的，电源
是倒的，而所有自检都是绿的（连接关系、坐标、角度值都没错，错的只有人眼看到
的那个图形）。

真机标定（图纸空白处摆 rot 0/90/180/270 各一个，人眼确认）：

| 符号 | 朝上 | 朝下 |
|---|---|---|
| Power | **rot 0** | rot 180 |
| Ground | rot 180 | **rot 0** |

**两者都用 0 就各自正确。** 以后要定任何符号的朝向，在真机上摆一排标定，
不要从别的符号推 —— 符号图形的原生方向是画符号的人定的，没有统一约定。

标定时可用的客观指纹：符号自带的那个引脚，其 `rotation` 指向符号本体的方向。
Power 正确时引脚 `rotation=270`，Ground 正确时 `rotation=90`。

#### 判断器件朝向：用引脚，不要用位号文字

位号文字是独立图元，`eda.sch_PrimitiveAttribute.getAll(器件primitiveId)` 能读到
带 `x/y` 的 `Designator` 项。看上去可以拿「文字在器件的哪一侧」反推朝向，
实测不行 —— 同一个电容转四个角度：

| 器件 rot | 位号文字偏移 | pin1 偏移 | pin1 rotation |
|---|---|---|---|
| 0 | (−10, 10) | (−20, 0) | 180 |
| 90 | (15, 0) | (0, −20) | 270 |
| 180 | (−10, 10) | (+20, 0) | 0 |
...
**文字位置只区分横竖，不区分正反**：0° 和 180° 完全一样，90° 和 270° 也一样。
而「电源朝上还是朝下」恰恰是 180° 之差，正是它分辨不了的那一类。镜像也不改变
文字位置（C1 `mirror=false` 与 C2–C5 `mirror=true` 的偏移都是 `(15,0)`）。

引脚才是可靠判据：四个角度四个值，且 `rotation` 由真机返回，不用自己算。
配合 y 轴向上为正，**`pin.rotation=90` 朝上、`270` 朝下**。
...
## §4.11 逐层递进：原理图应该一层层长出来，不是一次算完

### 现在这套模式错在哪

当前是「一次性」模式：AI 给一份完整地图 → 程序一把摆完所有器件 → 走线 →
最后放电源地符号和端口。

这个顺序把**所有约束堆到最后同时求解**，退火只能在一个巨大的解空间里找
折中。更要命的是**扇出排在最后**：器件都摆死了，符号只能在剩下的缝里挤。
于是就有了这一路的补丁 —— 落点占位、路径占位、加长避让、让不开就退回
贴引脚。每个补丁都在治症状，因为**避让的方向从一开始就是反的**：

> 芯片的引脚展开区是芯片的一部分，应该是**别人避开它**，
> 而不是它去躲别人。

一次性模式下做不到这件事，因为轮到扇出时，别人早就摆好了。

### 逐层递进

改成像人画图那样：放一点、连一点、看一眼、再放下一层。

```
第 1 层  放核心芯片 → 立刻扇出它的电源地与端口 → 连它的内部网络
         ↓ 这一层占掉的空间（器件 bbox ＋ 扇出区 ＋ 走线）成为硬约束
第 2 层  放外围器件（自然避开上一层的地盘）→ 连线
         ↓
第 3 层  ……
```
...
**一、扇出提前。** 核心芯片一放下就把电源地符号、端口摆好，这块区域随即
变成障碍。后面的器件是被布局算法挡在外面的，不需要扇出自己去躲。

**二、约束是递增的，不是同时的。** 每一层只在「前面层已定」的前提下求解
自己那一小块，解空间小得多，也更容易收敛到人能看懂的结果。

**三、每层都有回读与人（或 AI）的检查点。** 错了当场就能发现，而不是等
整张图画完再从一堆症状里倒推。这和 §0 铁律 8「写完必须回读」是同一条思路，
只是把粒度从「整张图」缩到「一层」。

### 对地图的影响：从整图声明变成增量更新

地图不再是「一次性交出的完整快照」，而是**逐层累积**的：

- `MapPart` 增加 `layer`：AI 声明这个器件属于第几层
- `eda_map_apply` 接受 `layer` 参数，只处理该层，并把**前面所有层已占的
  区域**作为障碍传给布局与布线
- 每层 apply 之后回读、体检，AI 看过再决定下一层怎么放

分工不变，仍然是 §4.8 那条：**AI 定语义（谁属于哪层、每层放什么），
程序定几何（在给定约束下摆放与布线）**。变的只是粒度 —— AI 现在还要决定
**顺序**，这恰好是它擅长而算法不擅长的判断。

### 布局内核要补的东西

1. **障碍输入**。`layoutByGroups` / `route` 目前只知道本次要摆的器件，
   需要能接受一组「已占矩形」，把它们当作不可侵入区。
2. **扇出纳入占地**。扇出区要计入该器件的有效包围盒，参与后续层的避让 ——
   这样「别人避开芯片展开区」才成立。
3. **增量渲染**。现在每次 apply 都清场重画，逐层模式下只能画新增的部分，
   否则前面层的成果每次都被抹掉。
4. **分叉点不能落在别人引出线的跨度内**。这是手摆 T6 的去耦电容时试出来的
   硬约束：从某根引出线上分叉往下引，垂直线会穿过下方所有横跨该 x 的引出线，
   **同侧相邻引脚往往一个是 VDD 一个是 VSS，一穿就是电源短到地**。
   实例：`pin48`(VDD_3) 想就近向下分叉，可它下方横着 `pin47`(VSS_3) 的引出线
   `y=415 x∈[345,535]`，分叉点必须挪到 `x=560`（超出那根线的末端）才安全；
   `pin9`(VDDA) 同理，`x≥85` 会撞 `pin23` 的线，只能选 `x=60`。
   算法目前没有这条判据，是我一个个手算的。
5. **就近器件要参与扇出布局，不能事后插空位**。去耦电容的全部意义在于紧贴
   它服务的那个电源引脚，可它在网表上和别的电容完全等价 —— 算法没有任何理由
   把 C1 放到 `pin24` 旁边而不是 `pin48` 旁边。地图缺的是**布局意图**，
   建议在 part 上加 `series: "U1.36"`（串在这个引脚的引出线上），扇出时把
   「引脚 → 分叉 → 电容 → 地符号」当成一个单元来排长度和朝向。

   为什么不能事后插：先把 5 个引脚的符号阶梯摆满，垂直空间就没了。实测 T6
   左侧 `x∈[-65,175]` 只剩 94 高的横带和 150 高的下方区域，而竖放一个带上下
   符号的电容需要约 210 高（符号45+引线40+本体40+引线40+符号45），塞不进去。
   结果 C4/C5 只能落到 `y≈110` 的底部，三条平行长线（215、295）从引脚一路
   拉下来 —— 电气上对，可「就近」这个语义已经丢干净了。这条同样适用于晶振
   负载电容、复位 RC、采样电阻：凡是位置本身就是设计一部分的器件都算。

### 为什么这也解决了可读性

人读原理图是按功能块读的。逐层生成天然产出按块聚集的图，而一次性退火
产出的是「全局代价最小」的图 —— 后者在数学上更优，在阅读上更差。
T5 那个被摆成两簇、相距 700 的 led 区就是例子：全局看代价不高，
人看就是散的。
