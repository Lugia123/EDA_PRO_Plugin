...
⚠️ **动手放器件之前，先读 `eda-schematic-layout` skill。**
那里讲的是怎么划功能区、各区摆图纸哪里、器件朝哪个方向 —— 不先规划就开摆，
画出来的图电气上对但人读不了。分区是你的判断，工具只负责执行。
...
### 画图的标准三步：放器件 → 声明网络 → 让 EDA 布线

```
eda_place_component   放器件（位置粗略即可）
eda_label_pin_net     声明每个引脚属于哪个网络 —— 只表达电气意图，不管几何
eda_auto_route        让 EDA 的布线算法把线画好看
```

**分工的道理**：AI 定义「谁连谁」，几何交给程序。自己算路径既画不好看，
又会因为交叉误连（见下）。实测 3.4 秒就能把 154 段短引出线整理成 46 条正规连线，
带拐弯、节点和避让 —— 是人能直接看懂的原理图。

从零画图、还没想好布局时，可以先随意放，再 `eda_auto_layout`（给 device_types
标注电阻/电容/芯片会排得更整齐），然后再 auto_route。
照着参考图摆位置的场合就别用 auto_layout，会打乱布局。
