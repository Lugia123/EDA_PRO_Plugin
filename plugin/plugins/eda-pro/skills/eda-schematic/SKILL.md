...
```
每个功能区：查引脚 → 摆核心 → 摆外围 → 声明网络 → auto_route → DRC → 下一区
```

**不要"全部放器件 → 全部连线 → 全部布线"**，那样方向必然乱、线只能绕。
一个区做完再做下一个，每次只关心十来个器件，摆得准也好查。

摆放时要让**要连的引脚彼此相对**（头对头），别让线绕过器件身子。
完整方法见 `eda-schematic-layout` skill。
